...
const data = [
  ...
  {title: '右下', child: [ {title: '点击打开', tap: 'onNotice', type: 'options', dire: 'bottom'}]},
]
              ...
              <View className='mt-default'>
                {
                  item.child.map((itemx, jj) => {
                    ...
                    return <Button key={'b-'+jj} title={itemx.title || item.title} itemClass={!item.type ? 'ss-button btn-default ml-default' : 'ml-default ss-i-block ss-link'} data-dire={itemx.dire} data-idx={ii+'_'+jj} data-type={itemx.type} tap={this[itemx.tap]} />
                  })
                }
              </View>
            </View>
...
export default function(Pager) {
  return Pager({
    ...
    onNotice(e, params, inst) {
      ...
      const ii = inst.attr('data-idx')
      const dire = inst.attr('data-dire')
      message.notice({
        title: '这是一条消息标题',
        body: ii+'这是最常用的弹出框样式，主要包括标题，文字内容以及按钮，文字大小14px，#666666 行高20px。',
        showClose: true,
